...
if (!file_exists($input)) die("Arquivo não encontrado.\n");
...
foreach ($artigos as $bloco) {
    ...
    foreach ($linhas as $linha) {
        ...
        if ($tag !== "  ") { $tagAtual = $tag; $campos[$tagAtual] = $val; }
        else { $campos[$tagAtual] .= " " . $val; }
    }
    ...
    foreach ($validadoresIA as $v) {
        if (strpos($textoTotal, $v) !== false) { $ehIA = true; break; }
    }
    if (!$ehIA) $status = "Ruido";

    $catFinal = "Nao Classificado";
    if ($status === "Valido") {
        foreach ($categorias as $nome => $termos) {
            foreach ($termos as $t) {
                if (strpos($textoTotal, $t) !== false) { $catFinal = $nome; break 2; }
            }
        }
    }
    ...
    if ($end !== "") {
        ...
        if (strpos($final, "CHINA") !== false) $final = "CHINA";
        if (strpos($final, "USA") !== false) $final = "USA";
    ...
    if ($status === "Ruido") $txtRuidos .= $linha;
    elseif ($catFinal === "Nao Classificado") $txtPendentes .= $linha;
    else $txtClassificados .= $linha;
}
